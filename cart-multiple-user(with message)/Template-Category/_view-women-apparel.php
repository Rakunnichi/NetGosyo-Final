<section class="py-5" id="view_top_products">
    <div class="text-center">
        ...
        <h4 class="font-rubik font-size-50"><b>Women`s Apparel</b></h4>
    </div>
    <div class="container px-4 px-lg-5 mt-5">
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">

            <?php
                    // $select_product = mysqli_query($conn, "SELECT * FROM `products` where item_brand = 'Women-Apparel' ORDER BY RAND()") or die('query failed!');
                    $select_product = mysqli_query($conn, "SELECT products.* FROM products LEFT JOIN user_form ON products.user_id = user_form.id
                    WHERE user_form.is_banned = 0 AND item_brand = 'Women-Apparel' ORDER BY products.sales DESC") or die('query failed!');
